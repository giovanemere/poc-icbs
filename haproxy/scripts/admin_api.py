...
def run_check_urls():
    ...
    try:
        # Ruta al script check-urls.sh
        script_path = "/scripts/check-urls.sh"
        
        # Si el script no existe en el contenedor, usar la ruta relativa
...
def api_url_status():
    """Endpoint para obtener el estado de las URLs."""
    try:
        return jsonify(get_url_status())
    except Exception as e:
        return jsonify({'error': str(e), 'urls': [], 'summary': {'success': 0, 'warnings': 0, 'errors': 0}})
