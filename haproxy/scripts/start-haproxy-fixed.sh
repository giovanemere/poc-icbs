...
# Modificar los scripts Python para usar puertos diferentes
sed -i 's/port=8081/port=9081/g' /etc/haproxy/scripts/admin_api.py
sed -i 's/port=8082/port=9082/g' /etc/haproxy/scripts/admin_ui.py

# Iniciar la API de administración en segundo plano (puerto interno 9081)
echo "Iniciando API de administración en puerto interno 9081..."
$PYTHON_CMD /etc/haproxy/scripts/admin_api.py &

# Iniciar la interfaz web en segundo plano (puerto interno 9082)
echo "Iniciando interfaz web de administración en puerto interno 9082..."
