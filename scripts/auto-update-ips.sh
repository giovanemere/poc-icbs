...
# =============================================================================
# Script de Actualización Automática de IPs Integrado
...
get_container_ip() {
    ...
    local network_name="${2:-weblogic-network}"
    
    # Intentar obtener IP del contenedor integrado primero
    local integrated_name="${container_name}-integrated"
    local ip=$(docker inspect "$integrated_name" 2>/dev/null | jq -r ".[0].NetworkSettings.Networks.\"config_${network_name}\".IPAddress" 2>/dev/null || echo "null")
    
    if [[ "$ip" == "null" || -z "$ip" ]]; then
        # Intentar con el nombre original
        ip=$(docker inspect "$container_name" 2>/dev/null | jq -r ".[0].NetworkSettings.Networks.\"config_${network_name}\".IPAddress" 2>/dev/null || echo "null")
    fi
    
    if [[ "$ip" == "null" || -z "$ip" ]]; then
        # Intentar con diferentes redes
        for net in "config_weblogic-network" "docker-for-oracle-weblogic_weblogic-network" "${network_name}"; do
            ip=$(docker inspect "$integrated_name" 2>/dev/null | jq -r ".[0].NetworkSettings.Networks.\"$net\".IPAddress" 2>/dev/null || echo "null")
            if [[ "$ip" != "null" && -n "$ip" ]]; then
                break
            fi
            ip=$(docker inspect "$container_name" 2>/dev/null | jq -r ".[0].NetworkSettings.Networks.\"$net\".IPAddress" 2>/dev/null || echo "null")
            if [[ "$ip" != "null" && -n "$ip" ]]; then
                break
            fi
        done
...
update_haproxy_config() {
    ...
    # También actualizar cualquier referencia a IPs específicas
    sed -i "s|172\.2[34]\.0\.[34]|${weblogic_a_ip}|g" "$config_file"
    sed -i "s|172\.2[34]\.0\.[23]|${weblogic_b_ip}|g" "$config_file"
...
main() {
    ...
    # Verificar que se obtuvieron todas las IPs
